...
module EVC
  ...
  class Summary
    ...
    def report!
      ...
      EVC.log.info "Summary Report:"

      @groups.sort.each do |id, group|
        items = group[:items].to_a

        EVC.log.info "  #{group[:id]} (#{group[:type]})"
        items.sort.each do |key, count|
          key = "<nil>" if key.nil?
          EVC.log.info "    #{key} #{count}"
        end
      end
  ...
  class Raw
    ...
    def report!
      ...
      EVC.log.info "Raw Report:"
      EVC.log.info "  count: #{@count}"
    ...
    def receive data
      if not @disabled and @count > 100
        ...
        if rate > @threshold
          ...
          EVC.log.info "Raw: disabled - rate too high (#{desc})"
          @disabled = true
        end
      end

      unless @disabled
        EVC.log.info "#{@count}: #{data}"
      end
  ...
  def self.main(args)
    ...
    EM.run do
      EM.connect(opts[:host], opts[:port], EVC::TraceConnection, handlers)
