...
public class UDP implements Protocol {
    ...
    public void bind(final String host, final int port, ChannelInitializer<DatagramChannel> initializer) {
    ...
    public void connect(final String host, final int port, ChannelInitializer<DatagramChannel> initializer) {
        final Bootstrap b = new Bootstrap();
        ...
        final ChannelRetrier.ChannelAction action = new ChannelRetrier.ChannelAction() {
            ...
            public void failed(int attempt, long delay, Throwable cause) {
                final long seconds = TimeUnit.SECONDS.convert(delay, TimeUnit.MILLISECONDS);
                log.info("Connect udp://{}:{} (attempt #{}) failed, reconnecting in {}s. Caused by: {}", host, port,
