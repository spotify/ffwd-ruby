...
public class FastForwardDecoder {
    ...
    public static void decode(ByteBufAllocator allocator, ByteBuf in, List<Object> out) throws Exception {
        while (in.readableBytes() >= 8) {
            ...
            if (totalLength > MAX_FRAME_SIZE) {
                throw new Exception("Received frame is larger than maximum allowed: " + totalLength);
            }
    ...
    private static Object decodeMetric0(final Protocol0.Metric metric) {
        ...
        return new Metric(metric.getKey(), metric.getValue(), time, host, tags, attributes, proc);
    }

    private static Map<String, String> convertAttributes0(List<Protocol0.Attribute> attributesList) {
    ...
    private static Object decodeEvent0(final Protocol0.Event event) {
        ...
        return new Event(event.getKey(), event.getValue(), time, ttl, state, description, host, tags, attributes);
    }
}
