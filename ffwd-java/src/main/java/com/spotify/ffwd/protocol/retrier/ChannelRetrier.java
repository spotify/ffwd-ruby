...
public class ChannelRetrier {
	...
	private static void tryAction(final Timer timer,
			final ChannelAction action, final int attempt) {
		...
		actual.addListener(new ChannelFutureListener() {
			...
			public void operationComplete(ChannelFuture future)
				...
				if (!future.isSuccess()) {
					final long delay = Math.min(MAX_TIMEOUT,
							(long) (TIMEOUT * Math.pow(2, attempt)));
